...
func pathsAddICQCmd(a *appState) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "icq [path-name] [status-bool] [buffer-int]",
		Aliases: []string{"n"},
		Short:   "Update the path to include interchainqueries and a chain height buffer to not attempt to submit results as they will timeout.",
		Args:    cobra.ExactArgs(3),
		Example: strings.TrimSpace(fmt.Sprintf(`
$ %s paths demo-path true 5
$ %s pth demo-path true 5`, appName, appName)),
		RunE: func(cmd *cobra.Command, args []string) error {
			pathName, icqBool, icqHeightBuffer := args[0], args[1], args[2]
			path, err := a.Config.Paths.Get(pathName)
			if err != nil {
				return fmt.Errorf("path needs to be configured before interchainqueries can be configured: %w", err)
			}
			boolValue, err := strconv.ParseBool(icqBool)
			if err != nil {
				return fmt.Errorf("path needs to be configured before interchainqueries can be configured: %w", err)
			}
			bufferValue, err := strconv.Atoi(icqHeightBuffer)
			if err != nil {
				return fmt.Errorf("path needs to be configured before interchainqueries can be configured: %w", err)
			}
			path.Interchainquery = relayer.Interchainquery{Query: boolValue, Buffer: bufferValue}

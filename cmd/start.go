...
$ %s start demo-path2 --max-tx-size 10`, appName, appName)),
		RunE: func(cmd *cobra.Command, args []string) error {
			c, src, dst, interchainquery, err := a.Config.ChainsFromPath(args[0])
			if err != nil {
				return err
			}
			...
			errorChan := relayer.StartRelayer(cmd.Context(), c[src], c[dst], interchainquery, filter, maxTxSize, maxMsgLength)

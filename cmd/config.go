...
func (c *Config) ChainsFromPath(path string) (map[string]*relayer.Chain, string, string, relayer.Interchainquery, error) {
	pth, err := c.Paths.Get(path)
	if err != nil {
		return nil, "", "", relayer.Interchainquery{}, err
	}

	src, dst, interchainquery := pth.Src.ChainID, pth.Dst.ChainID, pth.Interchainquery
	...
	return chains, src, dst, interchainquery, nil
}

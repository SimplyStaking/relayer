...
type RelayInterchainqueryMsgs struct {
...
// NewRelayMsgs returns an initialized version of interchainquery relay messages
func NewRelayInterchainqueryMsgs() *RelayInterchainqueryMsgs {
	return &RelayInterchainqueryMsgs{Msgs: []provider.RelayerMessage{}, Last: false, Succeeded: false}
}

// Ready returns true if there are messages to relay
func (r *RelayInterchainqueryMsgs) Ready() bool {
	if r == nil {
		return false
	}
...
func (r *RelayInterchainqueryMsgs) Success() bool {
	return r.Succeeded
}
...
func (r *RelayInterchainqueryMsgs) IsMaxTx(msgLen, txSize uint64) bool {
	return (r.MaxMsgLength != 0 && msgLen > r.MaxMsgLength) ||
		(r.MaxTxSize != 0 && txSize > r.MaxTxSize)
}
...
func (r *RelayInterchainqueryMsgs) Send(ctx context.Context, src, dst *Chain) {
	//nolint:prealloc // can not be pre allocated
	var (
		msgLen, txSize uint64
		msgs           []provider.RelayerMessage
